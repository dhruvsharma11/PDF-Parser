...
import fitz as pymupdf  # Updated import for pymupdf
import io
import base64
import pybboxes as pbx

pdf_path = "../pdf/Canada Price List 2024.pdf"


def pdf_to_images(pdf_path, output_dir="../pdf_images/"):
    images = convert_from_path(pdf_path, dpi=300, fmt="jpeg")
    train_dir = "../../train_model/data/images/train"

    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    for i, image in enumerate(images):
        image_filename = f"page_{i + 1}_thermasol.jpeg"

        output_path = os.path.join(output_dir, image_filename)
        train_path = os.path.join(train_dir, image_filename)
...
def extract_boundary_boxes(
    ...
    output_dir="../boundary/",
    results_dir="../results/",
    training_boxes_dir="../training_boxes/",
):
    ...
    if not os.path.exists(training_boxes_dir):
        os.makedirs(training_boxes_dir)
    ...
    for c in cnts:
        ...
        if w > 500 and h > 300:
            ...
            box = (x, y, w, h)
            yolo_box = pbx.convert_bbox(
                box, image_size=(width, height), from_type="coco", to_type="yolo"
            )
            yolo_boxes.append(
                f"0 {yolo_box[0]:.8f} {yolo_box[1]:.8f} {yolo_box[2]:.8f} {yolo_box[3]:.8f}"
            )
            ...
            for block in text_dict["blocks"]:
                if block["type"] == 1:
                    ...
                    if product_image_pil.mode == "RGBA":
                        product_image_pil = product_image_pil.convert("RGB")
                    image_filename = os.path.join(
                        img_dir, f"image_page_{page_num}_box_{index}_img_{count}.jpeg"
                    )
    ...
    if yolo_boxes:
        with open(f"{training_boxes_dir}/page_{page_num}_thermasol.txt", "w") as file:
            file.write("\n".join(yolo_boxes))
...
def process_images(directory, blur_size, kernel_size, edge_thresholds, pdf_document):
    for filename in os.listdir(directory):
        if filename.endswith(".jpeg"):
            ...
            page_num = int(os.path.splitext(filename)[0].split("_")[-2])
