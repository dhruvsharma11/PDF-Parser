...
def extract_boundary_boxes(
    ...
    for c in cnts:
        ...
        if w > 2000 and h > 300:
            ...
            text_dict = page.get_text("dict", clip=pdf_rect, sort=True)

            count = 0
            for block in text_dict["blocks"]:
                if block["type"] == 1:
                    print("new image" + str(block["size"]))
                    count += 1
                    ...
                    # Use absolute path and unique naming convention
                    image_filename = os.path.join(results_dir, f"image_page_{page_num}_box_{index}_img_{count}.jpeg")
